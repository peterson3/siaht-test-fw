...
{
    /// <summary>
    /// Classe de acesso ao Browser (E ao Driver)
    /// </summary>
    public class Browser
    {
        ...
        {
            get { return (IJavaScriptExecutor)Browser.Driver; }
        }
        ...
        {
            Browser.Action.MoveToElement(element).Click().Build().Perform();
        ...
        public static void getAllIFramesNames()
        {
            ...
            IReadOnlyCollection<IWebElement> elements = Browser.Driver.FindElements(By.TagName("iframe"));
        ...
        {
            Browser.Driver.SwitchTo().DefaultContent();
            Browser.Wait.Until(ExpectedConditions.FrameToBeAvailableAndSwitchToIt(iFrame));
            
            Browser.Wait.Until(ExpectedConditions.PresenceOfAllElementsLocatedBy(By.XPath("*")));


            IReadOnlyCollection<IWebElement> elements = Browser.Driver.FindElements(By.XPath("*"));
        ...
        {
            Browser.Driver.SwitchTo().DefaultContent();
            string   s = (string)Browser.JSexec.ExecuteScript("function getFrameSrc(){var a,b=document.getElementsByTagName('iframe');for(i=0;i<b.length;i++)'" + frameDesejado + "'==b[i].name&&(a=b[i].contentWindow.location.href);return a}var TESTE_AUTO = getFrameSrc();return TESTE_AUTO;");
            Match match = Regex.Match(s, @"((\w)+\.asp)");
            s = match.Value;
            return s;

            //Browser.Driver.SwitchTo().DefaultContent();
            //string s = (string)Browser.JSexec.ExecuteScript("function getFrameSrc(){var a,b=document.getElementsByTagName('iframe');for(i=0;i<b.length;i++)'" + frameDesejado + "'==b[i].name&&(a=b[i].src);return a}var TESTE_AUTO = getFrameSrc();return TESTE_AUTO;");
            ////Browser.JSexec.ExecuteScript("");
            ////return Browser.JSexec.ExecuteScript("").ToString();


            //if (String.IsNullOrWhiteSpace(s))
            //{
            //    s = (string)Browser.JSexec.ExecuteScript("function getFrameSrc(){var a,b=document.getElementsByTagName('iframe');for(i=0;i<b.length;i++)'" + frameDesejado + "'==b[i].name&&(a=b[i].contentWindow.location.href);return a}var TESTE_AUTO = getFrameSrc();return TESTE_AUTO;");
            //}
            //return s;
            ...
            //    try
            //    {
            //        s = (string)Browser.JSexec.ExecuteScript("function getFrameSrc(){var a,b=document.getElementsByTagName('iframe');for(i=0;i<b.length;i++)'" + frameDesejado + "'==b[i].name&&(a=b[i].contentWindow.location.href);return a}var TESTE_AUTO = getFrameSrc();return TESTE_AUTO.split('?')[0].split('/')[TESTE_AUTO.split('?')[0].split('/').length-1];");
        ...
        public static void reloadFrame(string frameDesejado)
        {
            Browser.JSexec.ExecuteScript("var b=document.getElementsByTagName('iframe');for(i=0;i<b.length;i++)'" + frameDesejado + "'==b[i].name&&b[i].contentWindow.location.reload();");
